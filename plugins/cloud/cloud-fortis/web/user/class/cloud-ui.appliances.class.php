...
var $cloud_id;
	...
	function select() {
		...
		if ( isset($_GET['table']) == true && $_GET['table'] == 'appliances_table') {
				...
				if(isset($appliance)) {
					...
					if ($sshterm_enabled) {
						if (($sshterm_login) && (isset($sshterm_login_ip))) {
							...
							*/
							$plugin_action = '<li class="list-group-item"><a href="#" onclick="sshwindow = window.open(\''.$sshterm_login_url.'\',\''.$sshterm_window.'\', \'location=0,status=0,scrollbars=yes,resizable=yes,width=973,height=500,left=100,top=100,screenX=400,screenY=100\'); sshwindow.focus(); return false;"><i class="fa fa-terminal"></i>&nbsp;' .  $this->lang['appliances']['plugin_ssh'] . '</a></li>';
	...
	*/
					...
					if ($show_pause_button) {
						...
						*/
						$cloudappliance_action .= '<li rel="' . $this->response->get_url($this->actions_name, 'pause').'&'.$this->identifier_name.'[]='.$this->cloudappliance->id . '" class="list-group-item"><span><i class="fa fa-pause"></i>&nbsp;' .  $this->lang['appliances']['action_pause'] . '</span></li>';
					...
					if ($collectd_graph_enabled) {
						...
						if(!empty($appliance->id)){
							...
							if (file_exists($data)) {
								...
								$plugin_action .= $a->get_string();
								*/
								$plugin_action .= '<li rel="' . $this->response->get_url($this->actions_name, 'statistics').'&appliance_id='.$appliance->id . '" class="list-group-item"><span><i class="fa fa-bar-chart"></i>&nbsp;Statistics</span></li>';
							}
						} else {
					...
					if (!strcmp($this->cloudconfig->get_value_by_key('allow_vnc_access'), "true") && $show_pause_button) {
						...
						$plugin_action .= $a->get_string();
						*/
						$plugin_action .= '<li class="list-group-item"><a href="api.php?action=novnc&appliance_id='.$appliance->id . '" target="_blank"><i class="fa fa-laptop"></i>&nbsp;' . $this->lang['appliances']['plugin_novnc'] .'</a></li>';
					}
					...
					$ta[] = array(
						...
						'action' => $cloudappliance_action.$plugin_action.$cdrom,
					);
					$app_count++;
				} else {
					$this->cloudrequest->get_instance_by_id($reqid);
